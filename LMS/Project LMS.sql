...
select * from loan_sta;

-- customer Grade 

create table CustomerGrade as select Customer_id,ApplicantIncome,
case
...
else 'Low Class'end as customergrade from cus_inc;

select * from customergrade;

-- Monthly interest percentage 

create table customer_income_analysis select * ,
case 
when applicantincome>15000 then 'GRADE A'
when applicantincome>9000 then 'GRADE B'
when applicantincome>5000 then 'MIDDLE CLASS CUSTOMER'
...
end as monthly_interest_percentage from cus_inc;

select * from customer_income_analysis;
...
select * from loan_sta;
describe loan_sta;
delete from loan_sta;


delimiter //
create trigger loan_amount_status before insert on loan_sta
for each row
begin
...
create table cibil_score_status_details(loan_id varchar(55), loanamount varchar(55), cibil_score int, cibil_score_status varchar(65), primary key(loan_id));


delimiter //
create trigger cibil_score_update after insert on loan_sta
for each row
begin
...
alter table cibil_score_status_details modify loanamount int;

--  create all the above fields as a table
create table loan_cibil_score_status_details as select l.*,c.cibil_score_status
from loan_sta l inner join cibil_score_status_details c on l.loan_id=c.loan_id;

select * from loan_cibil_score_status_details;
describe customer_income_analysis;

-- update gender and age based on customer id

update cus_inc set gender = 'female' where customer_id='IP43006';
update cus_inc set gender = 'female' where customer_id='IP43016';
update cus_inc set gender = 'male' where customer_id='IP43018';
update cus_inc set gender = 'male' where customer_id='IP43038';
update cus_inc set gender = 'female' where customer_id='IP43508';
update cus_inc set gender = 'female' where customer_id='IP43577';
update cus_inc set gender = 'female' where customer_id='IP43589';
update cus_inc set gender = 'female' where customer_id='IP43593';
update cus_inc set age = 45 where customer_id='IP43007';
update cus_inc set age = 32 where customer_id='IP43009';
...
create table customer_interest_analysis select c.*,l.loanAmount,l.loan_Amount_term,l.cibil_score, l.cibil_score_status,
(c.monthly_interest_percentage*l.loanamount)/100 as monthly_interest_amount,
(c.monthly_interest_percentage*l.loanamount*12)/100 as annual_interest_amount  
from customer_income_analysis c inner join loan_cibil_score_status_details l on c.loan_id=l.loan_id;

alter table customer_interest_analysis change Customer_ID  Customer_id varchar(50);
alter table cus_det change Customer_ID  Customer_id varchar(50);
alter table customer_income_analysis change Customer_ID  Customer_id varchar(50);

select * from customer_interest_analysis;


create table customer_income_status select Loan_ID, Customer_id,ApplicantIncome,CoapplicantIncome,Property_Area,Loan_Status,customergrade,concat(monthly_interest_percentage,'%') as monthly_interest_percentage from customer_income_analysis;

select * from customer_income_status;


-- join all the 5 tables without repeating  the feilds - output1

create table output1 select s.*,c.loanAmount,c.loan_Amount_term,c.cibil_score,c.cibil_score_status,c.monthly_interest_amount,c.annual_interest_amount, 
d.customer_name, d.Gender,d.Age,d.Married,d.Education,d.Self_Employed,d.Region_id,
e.Postal_code,e.segment,e.state, 
f.region from 
customer_income_status s inner join customer_interest_analysis c on s.customer_id=c.customer_id 
inner join cus_det d on c.customer_id=d.customer_id 
inner join cou_sta e on d.customer_id=e.customer_id 
inner join reg_info f on d.region_id=f.region_id;

-- Connecting tables for ER diagram
...
describe loan_sta;
describe customer_income_analysis;
describe cibil_score_status_details;
describe output1;


alter table cus_det modify Customer_id varchar(50) primary key;
alter table cou_sta modify Customer_id varchar(50) primary key;
alter table cou_sta add constraint custID_key foreign key(Customer_id) references cus_det(Customer_id);
alter table output1 add constraint out_key foreign key(Customer_id) references cus_det(Customer_id);
alter table cus_inc change Customer_ID  Customer_id varchar(50);
alter table cus_inc add constraint cusID_key foreign key(Customer_id) references cus_det(Customer_id);
alter table cus_inc modify Loan_id varchar(35);
alter table cus_inc add constraint loanID_key foreign key(Loan_id) references loan_sta(Loan_id);
alter table reg_info modify Region_Id double primary key;
alter table cou_sta add constraint region_key foreign key(Region_Id) references reg_info(Region_Id);
alter table output1 add constraint region_keys foreign key(Region_Id) references reg_info(Region_Id);
alter table customer_income_analysis add constraint custrID_key foreign key(Customer_id) references cus_det(Customer_id);
alter table loan_cibil_score_status_details add constraint loanID1_key foreign key(Loan_id) references loan_sta(Loan_id);
alter table cus_inc add constraint loanID_key foreign key(Loan_id) references loan_sta(Loan_id);

select * from output1;


 -- find the mismatch details using joins - output2

select* from cou_sta c right join reg_info r on c.region_id=r.region_id where c.region_id is null;

-- Filter high cibil score - output 3

 select * from output1 where cibil_score_status='high cibil score';


 -- Filter home office and corporate - output4

 select * from output1 where segment in('home office', 'corporate');
...
begin
select * from output1;
select* from country_state c right join reg_info r on c.region_id=r.region_id where c.region_id is null;
select * from output1 where cibil_score_status='high cibil score';
select * from output1 where segment in('home office', 'corporate');
end //
delimiter ;
...
use project;
select count(*) as final_records_count from output1;